...
RSpec.describe IssueHistoriesController, type: :controller do
  context 'GET' do
    ...
    it 'should not be a valid Edit Action' do 
      issue_history = FactoryGirl.create(:issue_history)
      get :edit, id: Faker::Company.name , format:'json', issue_history:{ issue_type:'', issue_date:'', return_date:'', member_id:'', book_id:'' }
      response.should have_http_status(:unprocessable_entity)
    end 
    it 'should not be a valid Edit Action' do 
      issue_history = FactoryGirl.create(:issue_history)
  ...
  context 'POST' do 
    it 'should be a valid Create Action' do
      ...
      post :create, format:'json', issue_history:{ issue_type:'rent', issue_date:Faker::Date.between(2.days.ago, Date.today).strftime("%F"), return_date:Faker::Date.between(2.days.ago, Date.today).strftime("%F"), member_id:member.id, book_id:book.id }
      response.should have_http_status(:ok)
    end
